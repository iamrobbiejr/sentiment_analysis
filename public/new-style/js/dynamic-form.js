...
(function($) {
    ...
    $.fn.dynamicForm = function(
        ...
        function innerClickOnMinus(event) {
            ...
            checkId = $.inArray(
                parseInt(removableCloneId[removableCloneId.length - 1]),
                subDynamicIndex
            );
        ...
        $(this)
            ...
            .each(function() {
                if (
                    $.isFunction(this.addSubDynamicForm) &&
                    !options.isSubDynamicForm
                ) {
                    isMainForm = false;
                    ...
                    formPrefix =
                        this.getFormPrefix() + "[0][" + formPrefix + "]";
                    return false;
                }
            });
        ...
        isPlusDescendentOfTemplate =
            isPlusDescendentOfTemplate.length > 0 ? true : false;
                ...
                function fillData(formIndex, formValue) {
                    ...
                    $.each(formValue, function(index, value) {
                        if ($.isArray(value)) {
                            ...
                            if (
                                typeof mainForm.get(0).getSource === "function"
                            ) {
                                $.each(
                                    value,
                                    $.proxy(
                                        fillData,
                                        mainForm.get(0).getSource()
                                    )
                                );
                            }
                        } else {
                            var formElements = changeArray[formIndex].find(
                                "[origname='" + index + "']"
                            );
                            if (formElements) {
                                if (
                                    formElements.get(0).tagName.toLowerCase() ==
                                    "input"
                                ) {
                                    /* Fill in radio input */
                                    if (formElements.attr("type") == "radio") {
                                        ...
                                        formElements.attr("type") == "checkbox"
                                    ) {
